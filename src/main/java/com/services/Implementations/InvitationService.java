...
public class InvitationService implements IInvitationService {
	...
	public String add(Invitation invitation) {
		...
		if (companyExists == null) {
			...
			if (companyExists.getRole() != Role.COMPANY) {
				...
				String link = "http://localhost:4200/signUpEmployee";
				emailService.send(invitation.getPour(), invitation.getSujet(), emailService.buildEmail(invitation.getPour(),
						"click on the below link to sign up:", link));
	...
	public String delete(Invitation invitation) {
		String msg = "";
		Invitation ExistsBySujet = invitationRepository.findInvitationBySujet(invitation.getSujet());
		if (ExistsBySujet != null) {
			invitationRepository.delete(ExistsBySujet);
	...
	public String activateInvitation(String sujet){
		invitationRepository.activate(sujet, Status.ACTIVE);
